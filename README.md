...
**Cursor 协作**：Agent Skill **`claude-code-multi-agent-harness`** 为**独立仓库**，见 **[github.com/Jcxu97/claude-code-multi-agent-harness](https://github.com/Jcxu97/claude-code-multi-agent-harness)**（**非** VLV 运行时代码）。安装：将该项目克隆到 **`~/.cursor/skills/claude-code-multi-agent-harness/`**，详见该仓库 README；可选在 Cursor **Settings → Rules → User Rules** 加一句说明以强化习惯。
