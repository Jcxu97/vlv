...
DEFAULT_MERGED = ROOT / "out" / "transcript_merged.txt"
DEFAULT_OUT = ROOT / "out" / "video_analysis.txt"
...
def main() -> None:
    ...
    ap.add_argument(
        "--llm-provider",
        choices=("auto", "none", "gemini", "groq", "openai", "anthropic", "xai"),
        default="auto",
        help="auto：按 Gemini→OpenAI→Groq→Anthropic→xAI 使用首个已配置 Key；none 同 --no-llm",
    )
    args = ap.parse_args()
    ...
    if use_llm:
        ...
        if prov:
            try:
                ...
                print(f"已使用在线大模型（{prov}）生成分析。", flush=True)
            except Exception as e:
                print(f"[警告] 大模型调用失败，已改用本地规则：{e}", flush=True)
        elif args.llm_provider in (
            "gemini",
            "groq",
            "openai",
            "anthropic",
